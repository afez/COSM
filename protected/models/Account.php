...
class Account extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('account_username, account_type, account_password', 'required'),
			array('user_id', 'numerical', 'integerOnly'=>true),
			array('account_type, account_username, account_password', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, account_type, account_username, account_password, user_id', 'safe', 'on'=>'search'),
		);
	}
